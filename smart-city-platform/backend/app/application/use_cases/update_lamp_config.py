"""
UpdateLampConfigUseCase — admin-only threshold updates, recorded on the
blockchain as a config_change event.
"""
...
class UpdateLampConfigUseCase:
    ...
    async def execute(
        self, device_id: str, config: LampConfig, actor: str, now: datetime
    ) -> LampNode:
        lamp = await self._lamp_repository.get(device_id)
        if lamp is None:
            raise ValueError(f"Unknown lamp: {device_id}")

        updated_lamp = dataclasses.replace(lamp, config=config)
